-- 这些是安全的参数化查询示例，实际使用时应使用预处理语句来避免SQL注入
-- 1. 用户登录 (参数化查询示例)
...
-- 以下是修复后的安全示例查询（使用变量代替硬编码值，并添加权限检查）
SET @test_user_id = 1;
SET @test_album_id = 1;
SET @test_photo_id = 2;
...
-- 4. 查询某用户所有相册 (安全示例)
SELECT ResID, ResType, NameLink, Ext1, Ext3, CreateTime, UpdateTime FROM Resource 
WHERE UserID = @test_user_id AND ResType = 1;

-- 5. 查询某相册下所有照片 (安全示例)
SELECT ResID, ResType, NameLink, Ext1, Ext2, Ext3, CreateTime, UpdateTime FROM Resource 
WHERE UserID = @test_user_id AND ResType = 2 AND RelateID = @test_album_id ORDER BY CreateTime DESC;
...
-- 7. 按标签检索照片 (安全示例)
SELECT ResID, ResType, NameLink, Ext1, Ext2, Ext3, CreateTime, UpdateTime FROM Resource 
WHERE UserID = @test_user_id AND ResType = 2 AND Ext1 LIKE CONCAT('%', @test_tag, '%');

-- 安全查询：检查用户权限（推荐在所有查询中加入权限检查）
SELECT r.ResID, r.ResType, r.NameLink, r.Ext1, r.Ext2, r.Ext3, r.CreateTime, r.UpdateTime FROM Resource r
...
-- 更安全的查询示例：使用存储过程
DELIMITER //
CREATE PROCEDURE GetUserAlbums(IN input_user_id INT)
BEGIN
    SELECT ResID, ResType, NameLink, Ext1, Ext3, CreateTime, UpdateTime 
    FROM Resource 
    WHERE UserID = input_user_id AND ResType = 1;
END //
DELIMITER ;

DELIMITER //
CREATE PROCEDURE GetAlbumPhotos(IN input_user_id INT, IN input_album_id INT)
BEGIN
    SELECT ResID, ResType, NameLink, Ext1, Ext2, Ext3, CreateTime, UpdateTime 
    FROM Resource 
    WHERE UserID = input_user_id AND ResType = 2 AND RelateID = input_album_id 
...
BEGIN
    SELECT UserID, UserName, NickName, RemainingStorage, Role, CreateTime, UpdateTime
    FROM Users
...
DELIMITER //
CREATE PROCEDURE GetUserByUserName(IN input_username VARCHAR(20))
BEGIN
    SELECT UserID, UserName, NickName, RemainingStorage, Role, CreateTime, UpdateTime
    FROM Users
